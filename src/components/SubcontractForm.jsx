...
const SubcontractForm = ({ projectId, initialData = null, onSubmit, onCancel }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-gray-50 rounded-lg border border-gray-200 shadow-sm animate-in fade-in-50">
      <div className="space-y-2">
        <Label htmlFor="desc" className="text-sm font-semibold text-gray-700">Subcontract Description</Label>
        <Textarea
          id="desc"
          value={formData.description}
          onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))}
          placeholder="E.g. External backend development module..."
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <Button type="submit" size="sm" disabled={isSubmitting} className="h-8 bg-blue-600 hover:bg-blue-700 text-white">
          ...
          {initialData ? 'Update Subcontractor' : 'Save Subcontractor'}
        </Button>
      </div>
    </form>
  );
